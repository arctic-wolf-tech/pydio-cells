...
const supportedLanguages = [ <
    ...
    MenuItem key = { "pt-br" }
    value = { "pt-br" }
    primaryText = { "Português do Brasil" }
    />
]
...
class InstallForm extends React.Component {
    ...
    renderS3BucketsList() {
        ...
        if (canCreate) {
            ...
            result.Component = ( <
                span >
                <
                FontIcon className = { "mdi mdi-check" }
                color = { "#2e7d32" }
                style = {
                    { fontSize: 'inherit' }
                }
            ...
            if (notExist.length) {
                result.Valid = false;
                result.Component = < div > < FontIcon className = { "mdi mdi-alert" }
                color = { "#c62828" }
                style = {
                    { fontSize: 'inherit' }
                }
                /> {this.t('form.bucketList.notFound')} : {notExist.join(', ')}. <span style={{color:'"#c62828"'}}>{this.t('form.bucketList.warnCreate')}</span > < /div>
            } else {
                result.Valid = true;
                result.Component = < div > < FontIcon className = { "mdi mdi-check" }
                color = { "#2e7d32" }
                style = {
                    { fontSize: 'inherit' }
                }
                /> {this.t('form.bucketList.found')} : {newBuckets.join(', ')}.</div >
            }
        }
        return result;
    }
    ...
    renderStepActions(step, nextDisabled = false, leftAction = null) {
        ...
        return ( <
            div style = {
                { margin: '12px 0', display: 'flex', alignItems: 'center' }
            } > { leftAction } <
            span style = {
                { flex: 1 }
            }
            ...
            div > {
                step > 0 && ( <
                    FlatButton label = { this.t('stepper.button.back') }
            ...
            RaisedButton label = { stepIndex === 3 + stepOffset ? this.t('stepper.button.last') : this.t('stepper.button.next') }
            primary = { true }
            onClick = { nextAction }
    ...
    render() {
        ...
        if (stepIndex === 0) {
            leftAction = ( <
                div >
                <
                Checkbox checked = { licenseAgreed }
                label = { this.t('welcome.agreed') }
                style = {
                    { width: 300 }
                }
                onCheck = {
                    () => { this.setState({ licenseAgreed: !licenseAgreed }) }
                }
                /> < /
                div >
            );
        ...
        if (licenseRequired) {
            ...
            additionalStep = ( <
                    ...
                    <
                    div style = {
                        { padding: '20px 0', color: '#388E3C', fontSize: 14 }
                    ...
                    licCheckFailed &&
                        <
                        div style = {
                            { color: '#E53935', paddingTop: 10, fontWeight: 500 }
                        } > { this.t('license.failed') } < /div>
                } {
                    !licCheckPassed &&
                        <
                        div > { this.t('license.required') } < a href = { "mailto:services@pydio.com" } > services @pydio.com < /a>. <
                    Field name = "licenseString"
                    component = { renderTextField }
                    floatingLabel = { this.t('license.fieldLabel') }
                    label = { this.t('license.fieldLegend') }
                    multiLine = { true }
                    rowsMax = { 15 }
                    /> < /
                    div >
    ...
    steps.push( <
        ...
        pre style = {
            { height: 350, border: '1px solid #CFD8DC', borderRadius: 2, backgroundColor: '#ECEFF1', padding: 10, overflowY: 'scroll', lineHeight: '1.4em' }
        } > { agreementText } <
        /pre> < /
        div > { this.renderStepActions(0, !licenseAgreed, leftAction) } <
        /StepContent> < /
        Step > );
    ...
    steps.push( <
...

        {
            ...
            Field name = "dbUseDefaults"
            component = { renderInvertCheckbox }
            label = { < span > { this.t('database.forceConfigure') } < span style = {
                    { fontWeight: 500 }
                } > { this.t('database.legend.bold') }. < /span></span >
            }
            /> < /
            div >
        } {
            !dbUseDefaultsToggle &&
                <
                span > { this.t('database.legend') } < span style = {
                    { fontWeight: 500 }
                } > { this.t('database.legend.bold') }. < /span> < /
            span >
        } {
            dbCheckError &&
                <
...

        {
            ...
            div style = { flexContainer } >
                <
                Field name = "dbConnectionType"
            component = { renderSelectField } >
                <
                MenuItem value = "tcp"
            primaryText = { this.t('form.dbConnectionType.tcp') }
            /> <
            MenuItem value = "socket"
            primaryText = { this.t('form.dbConnectionType.socket') }
            /> <
            MenuItem value = "manual"
            primaryText = { this.t('form.dbConnectionType.manual') }
            /> < /
            Field >

                {
                    dbConnectionType === "tcp" && ( <
                        ...
                        <
                        div style = {
                            { flex: 4, marginRight: 2 }
                        } > < Field name = "dbTCPHostname"
                        component = { renderTextField }
                        floatingLabel = { this.t('form.dbTCPHostname.label') }
                        label = { this.t('form.dbTCPHostname.legend') }
                        /></div >
                        <
                        div style = {
                            { flex: 1, marginLeft: 2 }
                        } > < Field name = "dbTCPPort"
                        component = { renderTextField }
                        floatingLabel = { this.t('form.dbTCPPort.label') }
                        label = { this.t('form.dbTCPPort.legend') }
                        /></div >
                        <
                        /div> <
                        Field name = "dbTCPName"
                        component = { renderTextField }
                        floatingLabel = { this.t('form.dbName.label') }
                        label = { this.t('form.dbName.legend') }
                        /> <
                        div style = {
                            { display: 'flex' }
                        } >
                        ...
                        } > < Field name = "dbTCPUser"
                        component = { renderTextField }
                        floatingLabel = { this.t('form.dbUser.label') }
                        label = { this.t('form.dbUser.legend') }
                        /></div >
                        <
                        div style = {
                            { flex: 1, marginLeft: 2 }
                        } > < Field name = "dbTCPPassword"
                        component = { renderPassField }
                        floatingLabel = { this.t('form.dbPassword.label') }
                        label = { this.t('form.dbPassword.legend') }
                        /></div >
                        <
                        /div> < /
                        div >
...

            {
                dbConnectionType === "socket" && ( <
                    div style = { flexContainer } >
                    <
                    Field name = "dbSocketFile"
                    component = { renderTextField }
                    floatingLabel = { this.t('form.dbSocketFile.label') }
                    label = { this.t('form.dbSocketFile.legend') }
                    defaultValue = "/tmp/mysql.sock" / >
                    <
                    Field name = "dbSocketName"
                    component = { renderTextField }
                    floatingLabel = { this.t('form.dbName.label') }
                    label = { this.t('form.dbName.legend') }
                    defaultValue = "pydio" / >
...

            {
                dbConnectionType === "manual" && ( <
                    div style = { flexContainer } >
                    <
                    Field name = "dbManualDSN"
                    component = { renderTextField }
                    floatingLabel = { this.t('form.dbManualDSN.label') }
                    label = { this.t('form.dbManualDSN.legend') }
                    /> < /
                    div >
                )
            } <
...

        {
            tablesFound &&
                ...
                div >
                <
                Checkbox checked = { tablesFoundConfirm }
            onCheck = {
                (e, v) => { this.setState({ tablesFoundConfirm: v }) }
            }
            /> < /
            div > <
                div style = {
            ...
            onClick = {
                (e) => {
                    e.preventDefault();
                    e.stopPropagation();
                    this.setState({ dbCheckSuccess: null, tablesFoundConfirm: null })
                }
            } > { this.t('database.installDetected.retry') } < /a>. < /
            div > <
                /div>
        } <
        /div> { this.renderStepActions(1 + stepOffset) } < /
        StepContent > <
        /Step>
    );
    ...
    steps.push( <
            ...
            div style = { stepperStyles.contentScroller } >
            <
            h3 > { this.t('admin.title') } < /h3> { this.t('admin.legend') }

            <
            div style = { flexContainer } >
            <
            Field name = "frontendApplicationTitle"
            component = { renderTextField }
            floatingLabel = { this.t('form.frontendApplicationTitle.label') }
            label = { this.t('form.frontendApplicationTitle.legend') }
            /> <
            Field name = "frontendDefaultLanguage"
            component = { renderSelectField }
            label = { this.t('form.frontendDefaultLanguage.label') } > { supportedLanguages } < /Field> {
            adminFound &&
            <
            div style = {
                { marginTop: 10 }
            ...
            <
            Checkbox checked = { adminFoundOverride }
            onCheck = {
                (e, v) => { this.setState({ adminFoundOverride: v }) }
            }
            ...
            div >
                <
                Field name = "frontendLogin"
            component = { renderTextField }
            floatingLabel = { this.t('form.frontendLogin.label') }
            label = { this.t('form.frontendLogin.legend') }
            /> <
            div style = {
                    { display: 'flex' }
                } >
                ...
                <
                Field name = "frontendPassword"
            component = { renderPassField }
            floatingLabel = { this.t('form.frontendPassword.label') }
            label = { this.t('form.frontendPassword.legend') }
            /> < /
            div > <
                div style = {
                    { flex: 1, marginLeft: 5 }
                ...
                <
                Field name = "frontendRepeatPassword"
            component = { renderPassField }
            floatingLabel = { this.t('form.frontendRepeatPassword.label') }
            label = { this.t('form.frontendRepeatPassword.legend') }
            /> < /
            div > <
                /div> < /
            div >
        ...
        /div> < /
    div > { this.renderStepActions(3 + stepOffset, !(adminFound || (frontendLogin && frontendPassword && frontendRepeatPassword))) } <
        /StepContent> < /
    Step >
);
...
steps.push( <
    ...
    div style = { stepperStyles.contentScroller } >
    <
    h3 > { this.t('advanced.title') } < /h3> { this.t('advanced.legend') }

    <
    Checkbox label = { this.t('advanced.mongo.title') }
    onCheck = {
        (e, v) => change('DocumentsDSN', v ? 'mongodb://localhost:27017/cells?maxPoolSize=20&w=majority' : '')
    }
    ...
    div style = {
        { paddingBottom: 20 }
    } > {!mongoDSNError && < div style = {
            { opacity: .7, width: 440 }
        } > { this.t('advanced.mongo.legend') } < /div>} {!!mongoDSNError && < div style = { { color: '#E53935' }
    } > { mongoDSNError } < /div>} <
    div style = {
        { display: 'flex', alignItems: 'flex-end' }
    } >
    <
    TextField value = { DSNURL.hostname }
    onChange = {
        (e, v) => changeDSN(DSNURL, 'hostname', v)
    }
    ...
    floatingLabelFixed = { true }
    /> <
    TextField value = { DSNURL.port }
    onChange = {
        (e, v) => changeDSN(DSNURL, 'port', v)
    }
    ...
    floatingLabelFixed = { true }
    /> <
    TextField value = { DSNURL.pathname.replace('/', '') }
    onChange = {
        (e, v) => changeDSN(DSNURL, 'pathname', '/' + v)
    }
    floatingLabelText = { this.t('advanced.mongo.db') }
    fullWidth = { true }
    floatingLabelFixed = { true }
    /> { performingCheck === 'MONGO' && < div style = { { minWidth: 48, height: 48, padding: 12, boxSizing: 'border-box' }
} > < CircularProgress size = { 20 }
thickness = { 2.5 }
/></div >
} <
div > {
        mongoDSNValid && < FontIcon className = { "mdi mdi-check" }
        color = { "#4caf50" }
        style = {
            { width: 25, height: 32, marginLeft: 10 }
        }
...
div > <
    ...
    <
    TextField value = { DSNURL.username }
onChange = {
    (e, v) => changeDSN(DSNURL, 'username', v)
}
...
floatingLabelFixed = { true }
/> <
TextField value = { DSNURL.password }
onChange = {
    (e, v) => changeDSN(DSNURL, 'password', v)
}
...
onChange = {
    (e, v) => changeDSN(DSNURL, 'authSource', v)
}
...
div style = {
    { display: 'flex', alignItems: 'center', height: 40, cursor: 'pointer', width: 478 }
}
onClick = {
...
div > {
        ...
        <
        MenuItem value = ""
        primaryText = { this.t('form.dsType.FS') }
        /> <
        MenuItem value = "S3"
        primaryText = { this.t('form.dsType.S3') }
        /> < /
        Field > <
        /div> {
        dsType !== 'S3' &&
        ...
        Field name = "dsFolder"
        component = { renderTextField }
        floatingLabel = { this.t('form.dsFolder.label') }
        label = { this.t('form.dsFolder.legend') }
        /> < /
        div >
    } {
        dsType === 'S3' &&
        ...
        div > <
            ...
            <
            Field name = "dsS3ApiKey"
        component = { renderTextField }
        floatingLabel = { this.t('form.dsS3ApiKey.label') }
        label = { this.t('form.dsS3ApiKey.legend') }
        errorText = {
            (s3CheckKeysError && s3CheckKeysError.error) || s3CheckBucketsError
        }
        /> < /
        div > <
            div style = {
            ...
            <
            Field name = "dsS3ApiSecret"
        component = { renderPassField }
        floatingLabel = { this.t('form.dsS3ApiSecret.label') }
        label = { this.t('form.dsS3ApiSecret.legend') }
        /> < /
        div > {
                performingCheck === 'S3_KEYS' && < div style = {
                    { width: 48, height: 48, padding: 12, boxSizing: 'border-box' }
                } > < CircularProgress size = { 20 }
                thickness = { 2.5 }
                /></div >
            } <
...
/div> { this.renderStepActions(3 + stepOffset) } < /
StepContent > <
    /Step>
);
...
steps.push( <
        ...
        div > <
        div style = {
            {...flexContainer, paddingRight: 20, paddingTop: 10, fontSize: 14, paddingBottom: 20 }
        } > {
            installEvents.map((e, i) => {
                ...
                style = {
                    ...
                    { flex: 1 }
                } > { e.data.Message } < /div>{icon}</div >
            })
        } <
        /div> {
        installPerformed && !serverRestarted &&
        <
        div > { this.t('apply.success') } <
        /div>
    } {
        installPerformed && serverRestarted &&
            <
            div > { this.t('apply.success.restarted').replace('%1', willReloadIn) } <
            /div>
    } {
        installError &&
            <
            div > { this.t('apply.error') } < br / > { this.t('apply.error.detail') } < br / > { installError } <
            /div>
    } <
    /div> {
installPerformed && serverRestarted &&
...
return ( <
    Paper zDepth = { 2 }
    style = {
        { width: 800, minHeight: panelHeight, margin: 'auto', position: 'relative', backgroundColor: 'rgba(255,255,255,0.96)' }
    } >
    <
    div style = {
        { width: 256, height: panelHeight, backgroundColor: 'rgb(94, 142, 174)', fontSize: 13, display: 'flex', flexDirection: 'column' }
    } >
    <
    div style = {
        {
    ...
    orientation = "vertical" > { steps } <
    /Stepper> < /
    form > <
    div style = {
        { height: 56, padding: '0px 16px' }
    } >
    <
    SelectField value = { lang }
    onChange = {
        (e, i, v) => { change('frontendDefaultLanguage', v), this.setState({ lang: v }) }
    }
    fullWidth = { true }
    labelStyle = {
    ...
    underlineStyle = {
        { display: 'none' }
    } > { supportedLanguages } < /SelectField> < /
    div > <
    /div> < /
    Paper >
);
